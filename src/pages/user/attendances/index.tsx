        ...
        <section className='bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden'>
          <div className='p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4'>
            <div>
              ...
              <p className='text-sm text-slate-500'>Daftar seluruh aktivitas pekerjaan Anda.</p>
            </div>
            <Link
                ...
                {attendances.map((item) => (
                  ...
                  >
                    ...
                    <td className='px-6 py-4'>
                      ...
                      <div className='text-xs text-slate-500 line-clamp-1'>{item.workDescription || '-'}</div>
                    </td>
                    <td className='px-6 py-4'>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.status === 'selesai' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                        {item.status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className='px-6 py-4 text-sm text-slate-500 italic max-w-37.5 truncate'>{item.note || '-'}</td>
                      ...
                      {item.status === 'sedang_dikerjakan' ? (
                        ...
                        >
                          <input
                            ...
                            type='time'
                            required
                            className='text-xs border border-slate-200 rounded px-2 py-1 focus:ring-1 focus:ring-emerald-500 outline-none'
                          />
                          <button className='bg-slate-800 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-black transition-colors font-semibold'>Selesai</button>
                        </form>
                      ) : (
